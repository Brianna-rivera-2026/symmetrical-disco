...
async def test_idempotency_key_partial_unique(db_session):
    ...
    # so the collision must be tested with a shared, non-null user_id. Ownership
    # is a bare cluster UID now (no local users table to FK to).
    owner = uuid.uuid4()
...
async def test_users_table_dropped_and_job_ownership_columns(pg_engine):
    """Migration 0009: the local `users` table is gone (auth moved to cluster
    TokenReview) and `jobs.user_id` is a bare, unconstrained cluster UID with
    a companion `user_name` display column — no FK to resolve."""
    from sqlalchemy import inspect
    ...
    async with pg_engine.begin() as conn:
        ...
        def check_ownership_schema(c):
            ...
            assert not any(fk["constrained_columns"] == ["user_id"] for fk in fks)
